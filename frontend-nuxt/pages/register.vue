...
const apiBaseUrl = (config.public.apiBaseUrl || '/api').replace(/\/api$/, '')
const googleLoginUrl = computed(() => `${apiBaseUrl}/oauth2/authorization/google`)
const githubLoginUrl = computed(() => `${apiBaseUrl}/oauth2/authorization/github`)
const linkedinLoginUrl = computed(() => `${apiBaseUrl}/oauth2/authorization/linkedin`)

const handleRegister = async () => {
  try {
    await authStore.register(name.value, email.value, password.value, role.value)
    navigateTo({ path: '/auth/confirm', query: { email: email.value } })
  } catch (error) {
...
<template>
  <div class="space-y-6">
    <div class="space-y-2 text-center md:text-left">
      <h1 class="text-3xl font-bold font-heading tracking-tight text-zinc-900 dark:text-white">{{ $t('auth.registerTitle') }}</h1>
      <p class="text-zinc-500 dark:text-zinc-400">{{ $t('auth.registerSubtitle') }}</p>
    </div>
    ...
    <form @submit.prevent="handleRegister" class="space-y-4">
      ...
      <Button type="submit" class="w-full bg-primary hover:bg-primary/90 text-primary-foreground shadow-[0_4px_14px_0_hsl(var(--primary)/0.39)] hover:shadow-[0_6px_20px_hsl(var(--primary)/0.23)] hover:-translate-y-0.5 transition-all duration-200" :disabled="authStore.loading">
        ...
        <span>{{ authStore.loading ? $t('common.loading') : $t('auth.registerButton') }}</span>
      </Button>
    </form>
    ...
    <div class="text-center text-sm text-zinc-500 dark:text-zinc-400 mt-6">
      ...
      <NuxtLink to="/login" class="font-medium text-primary hover:text-primary/80 hover:underline transition-all">
        {{ $t('auth.signIn') }}
      </NuxtLink>
    </div>
  </div>
