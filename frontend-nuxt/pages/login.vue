...
<template>
  <div class="space-y-6">
    ...
    <form @submit.prevent="handleLogin" class="space-y-4">
      ...
      <div class="space-y-2">
        <div class="flex items-center justify-between">
          ...
          <NuxtLink to="/auth/forgot-password" class="text-sm font-medium text-primary hover:text-primary/80 transition-colors">
    ...
    <div class="text-center text-sm text-zinc-500 dark:text-zinc-400 mt-6">
      {{ $t('auth.noAccount') }}
      <NuxtLink to="/register" class="font-medium text-primary hover:text-primary/80 hover:underline transition-all">
        {{ $t('auth.signUp') }}
      </NuxtLink>
    </div>
  </div>
