...
const publicProfileData = ref<any>(null)
...
const loadAllData = async () => {
  try {
    ...
    if (userId) {
      const data = await $api(`/public/profile/${userId}`)
...
const executeDelete = async () => {
  if (experienceToDelete.value) {
    try {
      await $api(`/professional/experiences/${experienceToDelete.value}`, { method: 'DELETE' })
        ...
        <div class="flex flex-col md:flex-row gap-6 items-center md:items-start">
          ...
          <div class="flex-1 text-center md:text-left space-y-2">
            ...
            <div class="pt-6 flex flex-wrap items-center justify-center md:justify-start gap-3">
              ...
              <Button 
                @click="navigateTo(`/u/${privateProfile?.username || authStore.user?.id}`)"
                variant="outline"
                size="sm"
