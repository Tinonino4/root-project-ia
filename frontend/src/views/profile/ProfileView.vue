...
const deleteExperience = async (expId) => {
  if (confirm('¿Estás seguro de que deseas eliminar esta experiencia laboral? Esto eliminará también cualquier feedback asociado de forma permanente.')) {
    try {
      await client.delete(`/professional/experiences/${expId}`);
...
<template>
  <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-8 min-h-screen">
    ...
    <div v-else-if="publicProfileData" class="space-y-8 animate-in fade-in-50 duration-500">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-white/5 pb-4">
        <div>
          <h2 class="text-3xl font-bold font-heading text-white">Mi Perfil</h2>
          <p class="text-zinc-400 text-sm mt-1">Vista previa interactiva. Así ven los reclutadores tu portafolio.</p>
        </div>
        
        <div class="flex flex-wrap items-center gap-3">
          <button 
            @click="copyProfileLink"
            class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-[hsl(228,15%,9%)] border border-white/5 text-zinc-300 hover:text-white hover:bg-[hsl(228,15%,12%)] transition-all duration-200 text-sm font-semibold"
          >
            <Copy class="w-4 h-4" />
            Compartir Perfil
          </button>
          ...
          <button 
            ...
            class="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-primary text-white hover:bg-primary/90 shadow-lg shadow-primary/20 transition-all duration-200 text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <Download class="w-4 h-4" :class="{'animate-bounce': isExporting}" />
            {{ isExporting ? 'PDF...' : 'Exportar PDF' }}
          </button>
        </div>
      </div>

      <!-- Hero Section -->
      <div class="bg-[hsl(228,15%,9%)] border border-white/5 rounded-2xl p-8 backdrop-blur-xl shadow-2xl relative group">
        <!-- Edit Profile Button -->
        <button 
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div class="lg:col-span-5 space-y-8">
          ...
          <div class="bg-[hsl(228,15%,9%)] border border-white/5 rounded-2xl p-6 backdrop-blur-xl shadow-xl relative group">
            <button 
              @click="router.push('/profile/edit')"
        ...
        <div class="lg:col-span-7">
          <div class="bg-[hsl(228,15%,9%)] border border-white/5 rounded-2xl p-6 backdrop-blur-xl shadow-xl">
            ...
            <!-- Experience Timeline -->
            <div v-if="publicProfileData.experiences && publicProfileData.experiences.length > 0" class="space-y-6 relative before:absolute before:inset-y-0 before:left-3 before:w-px before:bg-white/5">
              <div v-for="exp in publicProfileData.experiences" :key="exp.id" class="relative pl-8 group">
                <!-- Timeline dot -->
                <div class="absolute left-1.5 top-2.5 w-3 h-3 rounded-full bg-primary -translate-x-1/2 group-hover:scale-125 transition-transform duration-200"></div>
        ...
        <div class="relative z-10 flex-1 flex flex-col justify-between">
          <div class="space-y-8">
            ...
            <div class="flex gap-6 items-start bg-zinc-50 border rounded-2xl p-6" style="border-color: #e4e4e7;">
              ...
              >
                <span>{{ publicProfileData.name?.charAt(0) }}{{ publicProfileData.surname?.charAt(0) }}</span>
              </div>
              <div class="space-y-1.5 flex-1 min-w-0">
                <h3 class="text-xl font-extrabold text-zinc-950 truncate">{{ publicProfileData.name }} {{ publicProfileData.surname }}</h3>
                <p class="text-sm font-bold" style="color: #f29727;">{{ publicProfileData.jobTitle }}</p>
                <p class="text-xs text-zinc-600 leading-relaxed pt-1">{{ publicProfileData.aboutMe }}</p>
              </div>
            </div>
              ...
              <div v-if="publicProfileData.skills" class="grid grid-cols-2 gap-x-8 gap-y-4">
                <div class="space-y-1.5">
                  <div class="flex justify-between items-center text-xs font-bold text-zinc-800">
                    ...
                    <span>{{ (publicProfileData.skills.teamwork || 0).toFixed(1) }} / 5.0</span>
                  </div>
                  <div class="w-full bg-zinc-100 rounded-full h-2" style="background-color: #f4f4f5;">
                    <div class="h-2 rounded-full" :style="{ width: `${((publicProfileData.skills.teamwork || 0) / 5) * 100}%` }" style="background-color: #f29727;"></div>
                  </div>
                </div>
                ...
                <div class="space-y-1.5">
                  <div class="flex justify-between items-center text-xs font-bold text-zinc-800">
                    ...
                    <span>{{ (publicProfileData.skills.proactivity || 0).toFixed(1) }} / 5.0</span>
                  </div>
                  <div class="w-full bg-zinc-100 rounded-full h-2" style="background-color: #f4f4f5;">
                    <div class="h-2 rounded-full" :style="{ width: `${((publicProfileData.skills.proactivity || 0) / 5) * 100}%` }" style="background-color: #f29727;"></div>
                  </div>
                </div>
                ...
                <div class="space-y-1.5">
                  <div class="flex justify-between items-center text-xs font-bold text-zinc-800">
                    ...
                    <span>{{ (publicProfileData.skills.integrity || 0).toFixed(1) }} / 5.0</span>
                  </div>
                  <div class="w-full bg-zinc-100 rounded-full h-2" style="background-color: #f4f4f5;">
                    <div class="h-2 rounded-full" :style="{ width: `${((publicProfileData.skills.integrity || 0) / 5) * 100}%` }" style="background-color: #f29727;"></div>
                  </div>
                </div>
                ...
                <div class="space-y-1.5">
                  <div class="flex justify-between items-center text-xs font-bold text-zinc-800">
                    ...
                    <span>{{ (publicProfileData.skills.selfConfidence || 0).toFixed(1) }} / 5.0</span>
                  </div>
                  <div class="w-full bg-zinc-100 rounded-full h-2" style="background-color: #f4f4f5;">
                    <div class="h-2 rounded-full" :style="{ width: `${((publicProfileData.skills.selfConfidence || 0) / 5) * 100}%` }" style="background-color: #f29727;"></div>
                  </div>
                </div>
                ...
                <div class="space-y-1.5">
                  <div class="flex justify-between items-center text-xs font-bold text-zinc-800">
                    ...
                    <span>{{ (publicProfileData.skills.flexibility || 0).toFixed(1) }} / 5.0</span>
                  </div>
                  <div class="w-full bg-zinc-100 rounded-full h-2" style="background-color: #f4f4f5;">
                    <div class="h-2 rounded-full" :style="{ width: `${((publicProfileData.skills.flexibility || 0) / 5) * 100}%` }" style="background-color: #f29727;"></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="space-y-4">
              ...
              <div v-if="publicProfileData.experiences && publicProfileData.experiences.length > 0" class="space-y-4">
                <div 
          ...
          <div class="pt-8 border-t mt-12 flex justify-between items-end text-[9px] text-zinc-400" style="border-color: #e4e4e7;">
            ...
            <div class="text-right">
              ...
              <p class="font-mono">{{ authStore.user?.id.substring(0, 8) }}...{{ authStore.user?.id.substring(authStore.user?.id.length - 8) }}</p>
            </div>
          </div>
        </div>
      </div>
